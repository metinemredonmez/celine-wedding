...
| 4 | `/modeller` | Modeller | Dresses (index) | **MVP** |
| 5 | `/modeller/[dressSlug]` | Model / Gelinlik Detay | Dress Detail | **MVP** |
| 6 | `/randevu` | Randevu | Appointment | **MVP** |
| 7 | `/randevu/tesekkurler` | Randevu Onay | Appointment Confirmation | **MVP** |
| 8 | `/iletisim` | İletişim | Contact | **MVP** |
| 9 | `/hakkimizda` | Hakkımızda / Atölye | About / Atelier | **MVP** (kısa tutulabilir) |
| 10 | `/galeri` | Galeri | Lookbook / Gallery | **Faz 2** |
| 11 | `/sss` | Sıkça Sorulan Sorular | FAQ | **Faz 2** |
| 12 | `/kvkk`, `/gizlilik` | KVKK / Gizlilik | Privacy / KVKK | **Faz 2** (yasal, TR gerekli) |
| 13 | `/gelin-hikayeleri` | Gelin Hikâyeleri | Real Brides / Testimonials | Faz 3 |
| 14 | `/basin` | Basında Biz | Press | Faz 3 |
| — | `/admin/*` | Yönetim | Admin | **MVP** (bkz. [Admin](ADMIN.md)) |

> **Route standardı:** Bireysel gelinlik sayfası daima `/modeller/[dressSlug]` altındadır (butikte gelinliğe "model/gelinlik" denir). `/gelinlik` gibi eski adlar kullanılmaz.
...
1. **Full-bleed hero** — tek, sinematik, tam ekran (viewport) fotoğraf; alternatif olarak sessiz, otomatik oynayan sade bir video döngüsü. Üzerinde: marka logotype + tek satır marka sloganı. Alt-orta konumda ince bir "scroll" ipucu. Tek, sakin CTA: **"Randevu Alın"**. Bol negatif alan, sadelik.
...
5. **Editoryal hakkımızda şeridi** — split layout: bir yanda atölye/kurucu fotoğrafı, diğer yanda kısa, sıcak paragraf + "Hakkımızda" linki. El emeği/zanaat hissini verir.
...
### 2.8 Hakkımızda / Atölye (`/hakkimizda`) — MVP-lite

1. **Hero** — atölye veya kurucu portresi, full-bleed, + başlık "Atölye".
2. **Hikâye** — kurucu/marka hikâyesi, 2–3 paragraf, editoryal. Pull-quote.
3. **Zanaat şeridi** — görsel + metin: el işçiliği, kumaşlar, kişiye özel (made-to-measure) üretim.
4. **Süreç** — Randevu → Prova → Dikim → Teslim, görselli/ikonlu.
5. **Atölye görselleri** — 2–4 görselli mozaik.
6. **Randevu CTA bandı.**
7. **Footer.**

### 2.9 Galeri / Lookbook (`/galeri`) — Faz 2
...
- Sol veya orta: logotype.
- Sağ: nav — **Koleksiyonlar · Modeller · Galeri · Hakkımızda · İletişim · Randevu** (buton, dolgu/outline vurgulu).
...
- Dikey liste: Anasayfa, Koleksiyonlar, Modeller, Galeri, Hakkımızda, İletişim, **Randevu** (vurgulu).
...
| **2 — Menü** | Sayfa linkleri (Koleksiyonlar, Modeller, Galeri, Hakkımızda, İletişim, Randevu). |
...
**Anasayfa** (`home` singleton): `heroImage`/`heroVideo`, `heroTitle`, `heroSubtitle`, `heroCtaLabel`, `introText`, `featuredDresses[]` (→ Dress), `collectionsTeaser[]` (→ Collection), `aboutStrip{image, text, link}`, `processSteps[]`, `instagramFeed[]` (IG API/cache), `ctaBand{text}`.
...
**Hakkımızda** (`about` singleton): `heroImage`, `story` (rich text), `founderQuote`, `craftText`, `craftImage`, `processSteps[]`, `atelierImages[]`.
...
### Koleksiyon / Hakkımızda örnekleri
...
- **Faz 1 (MVP):** Anasayfa, Koleksiyonlar, Koleksiyon detay, Model/Gelinlik detay, Randevu (+onay), İletişim, kısa Hakkımızda. Global header/footer, WhatsApp FAB, randevu CTA bandları. Randevu formu → API + yönetici e-posta bildirimi + `wa.me` prefill fallback.
- **Faz 2:** Galeri/Lookbook, SSS, KVKK/Gizlilik + çerez banner'ı, tam admin dashboard, canlı Instagram besleme, lightbox, WhatsApp Business API/Twilio bildirim, form spam koruması.
- **Faz 3:** Gelin Hikâyeleri, Basında Biz, TR/EN çok dilli, gelişmiş SEO + schema.org, randevu için takvim/slot senkronizasyonu.
