...
export default async function DressDetailPage({ params }: PageProps) {
  const { slug } = await params;
  const [dress, settings] = await Promise.all([
    getDress(slug),
    getSiteSettings(),
  ]);
  ...
  const waText = `Merhaba, "${dress.name}" modeli için randevu almak istiyorum.`;
    ...
    <>
              ...
              {images.length > 0 ? (
                images.map((img, i) => (
                  <Reveal key={img.id ?? `${img.url}-${i}`} delay={i === 0 ? 0 : 0.05}>
                    <Media
                      src={img.url}
                      alt={img.alt ?? `${dress.name} — görsel ${i + 1}`}
                      ratio="portrait"
                      priority={i === 0}
                      className="bg-cream"
                    />
                ...
                <Reveal>
                  <div className="flex flex-col gap-6">
                    {collectionName ? (
                      ...
                      <span className="u-label text-rose">Couture</span>
                    )}
                    ...
                    <p className="text-muted leading-relaxed">
                      Ölçüye özel, atölyemizde el işçiliğiyle hazırlanan couture
                      bir tasarım. Her Celine gelinliği; kumaş, form ve detaylar
                      size göre yeniden yorumlanarak dikilir.
                    </p>
                    ...
                    <dl className="u-hairline flex flex-col gap-4 border-t pt-6">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <dd className="text-ink">Ölçüye özel dikim</dd>
                      </div>
                    </dl>
                    ...
                    <p className="u-hairline border-t pt-6 text-sm text-faint leading-relaxed">
                      Fiyat bilgisi tasarımın kumaş, işçilik ve detaylarına göre
                      değişir; randevu görüşmemizde birlikte netleştiririz.
                    </p>
                  </div>
                </Reveal>
              </div>
      ...
      {related.length > 0 ? (
        <section className="bg-cream py-20 sm:py-28">
          <Container>
            <Reveal>
              <SectionHeading eyebrow="Keşfet" title="Benzer Gelinlikler" />
            </Reveal>
